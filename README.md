<h1 align="center">Hi 👋, I'm Ali Hassan</h1>
<h3 align="center">Full Stack Developer | Building Scalable Web Apps</h3>

<p align="center">
  <a href="https://github.com/muhammadaliht">GitHub</a> •
  <a href="http://www.linkedin.com/in/ali-hassan-ah7">LinkedIn</a> •
  <a href="https://ali-hassan-portfolio-nine.vercel.app/">Portfolio</a>
</p>

---

## 🚀 About Me
- 💻 Full Stack Developer with real-world project experience  
- ⚡ Specialized in **React, Next.js & modern frontend UI/UX**  
- 🔗 Experienced in building **full-stack scalable applications**  
- 📍 Based in Pakistan  
- 🎯 Passionate about clean code, performance & user experience  

---

## 🛠️ Tech Stack

### 💻 Frontend
- React.js, Next.js, Redux  
- Tailwind CSS, Bootstrap, SCSS  
- Framer Motion, GSAP  

### 🔧 Backend
- Node.js, Express.js  
- Django, PHP  

### 🗄️ Database
- MongoDB, MySQL  

### ⚙️ Tools & Platforms
- Git & GitHub  
- Vercel, Netlify  
- REST APIs, GraphQL  

---

## 💼 Experience

### 🧑‍💻 MERN Stack Developer  
**AlgoMenta Technologies** *(Mar 2025 – Present)*  
- Built complete full-stack features from UI to backend  
- Developed APIs, authentication, and database systems  
- Worked with GraphQL & third-party integrations  

### 👨‍💻 Jr. Web Developer  
**Vantage Droid Pvt. Ltd** *(2023 – 2024)*  
- Created responsive UI with React & JavaScript  
- Handled backend logic using PHP & MySQL  
- Fixed server-side issues and optimized performance  

---

## 📌 Featured Projects

### 🧾 Digital Invoicing Platform
- Full-stack invoicing system with **FBR integration**
- Automated invoice lifecycle & validation  
🔗 https://digital-invoicing.com.pk/

---

### 🏢 Agency Eleva
- Secure authentication & role-based access system  
🔗 https://dev.agencyeleva.com/

---

### 📚 Muhammadan Quran School Portal
- Education management system with dashboards  
🔗 https://portal.muhammadanquranschool.com/

---

### 🎯 Other Projects
- Guidesly (Marketplace UI) → https://guidesly.com/  
- Dragon Cage → https://muhammadaliht.github.io/dragon-cage/  
- Stackly SaaS Landing → https://stackly.es/  

---

## 📫 Contact Me

- 📧 Email: muhammadali38906@gmail.com  
- 📞 Phone: 03126038906  

---

## ⚡ Fun Fact
I love solving real-world problems with code and building smooth user experiences 🚀
